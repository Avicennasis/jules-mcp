        ...
        artifacts: [
            {
                changeSet: {
                    ...
                    gitPatch: {
                        unidiffPatch: 'diff --git a/f.js b/f.js\n+the change',
                        baseCommitId: 'b',
...
describe('diff tools', () => {
    ...
    beforeEach(() => {
        ...
        mockClient = {
            ...
            listActivities: vi
                .fn()
                .mockResolvedValue({ activities: mockActivities }),
        };
    ...
    it('returns a structured error when the session is not found', async () => {
        ...
        (mockClient.getSession as any).mockRejectedValueOnce(
            new JulesNotFoundError('sessions/nope'),
        );
        const handler = registeredTools.get('jules_get_session_diff')!.handler;
