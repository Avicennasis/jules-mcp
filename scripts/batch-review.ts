...
for (const id of ids) {
  ...
  for (const art of last.artifacts) {
    ...
    for (const line of diff.split('\n')) {
      if (line.startsWith('diff --git')) files.push(line.replace('diff --git ', '').split(' ')[0].replace('a/', ''));
      else if (line.startsWith('+') && !line.startsWith('+++')) totalAdd++;
      else if (line.startsWith('-') && !line.startsWith('---')) totalDel++;
    }
  }
  ...
  for (const art of last.artifacts) {
    ...
    for (const line of lines) {
      ...
      if (inSrc && /^[+-@]/.test(line) && !/^[+-]{3}/.test(line)) out.push(line);
    }
    if (out.length) {
      console.log('--- PRODUCTION HUNKS ---');
      console.log(out.slice(0, 60).join('\n'));
      if (out.length > 60) console.log(`... (${out.length - 60} more lines)`);
    }
  }
}
