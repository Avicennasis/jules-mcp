...
console.log('=== By Repo ===');
for (const [repo, sessions] of [...byRepo.entries()].sort((a, b) => b[1].length - a[1].length)) {
  const states = new Map<string, number>();
  for (const s of sessions) {
    states.set(s.state, (states.get(s.state) ?? 0) + 1);
  }
  const stateStr = [...states.entries()].map(([s, c]) => `${s}: ${c}`).join(', ');
  ...
  if (s.outputs?.length) {
    for (const output of s.outputs) {
      if (output.pullRequest) {
        const repo = (s.sourceContext?.source ?? '').replace('sources/github/Avicennasis/', '');
...
for (const [repo, sessions] of [...byRepo.entries()].sort((a, b) => a[0].localeCompare(b[0]))) {
