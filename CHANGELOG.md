...
- **A `jules_run_task` poll timeout is a handoff, not an error (#50425).** When the deadline expires the response is now `status: TIMEOUT` (was `status: ERROR` / `code: 408` / `isError: true`) and carries the session id, the state it stopped in, and the exact `jules_get_session` call to resume with — so a session that legitimately runs for 20 minutes no longer reads as a failed call. Parallel mode already reported each session's outcome individually; its timeout note now names the exact resume call too.
