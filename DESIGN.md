...
| **Dedicated `jules_list_stuck_sessions` tool** | georgeracu        | Keep `jules_list_sessions(stale_only: true)` (#50426). It narrows by **state first** (so excluded sessions cost no API call), implies `detect_changes`, and deliberately excludes sessions whose activity fetch failed; the surveyed sweep passes no server-side `filter` at all and is entirely client-side. Their state set adds `AWAITING_PLAN_APPROVAL`, which ours skips by design — those are _approval-blocked_, not abandoned, and the existing `state:` filter already lists them. A named alias would be a second name for one filter. |
