...
- **Typed & tested**: TypeScript, 637 unit tests, smoke test against the live API.
...
| Tool                                  | Description                                                                                                                                                                                                                                                                                             | Key params                                                                                                                                                                                                                                                 |
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `jules_run_task` ✎                    | One-shot: create → poll → auto-approve → wait → return. Returns early if it needs input. Supports `parallel` (1–10) to fan out N independent sessions with the same prompt, matching the Jules CLI's `--parallel` flag. A poll that reaches `timeout_ms` returns `status: TIMEOUT` — with the session id, the state it stopped in, and the exact `jules_get_session` call to resume — rather than an error, because the session keeps running on Jules.                                                                                 | `prompt`, `source`, `starting_branch`, `title?`, `automation_mode?`, `reason`, `auto_approve?` (true), `poll_interval_ms?` (5000), `timeout_ms?` (600000), `parallel?` (1, max 10)                                                                         |
